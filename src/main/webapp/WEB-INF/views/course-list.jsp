            ...
            <c:if test="${not empty sessionScope.courseList}">
                <c:forEach var="course" items="${sessionScope.courseList}" varStatus="loop">
                    ...
                    <tr class="align-middle">
                        ...
                        <fmt:formatDate value="${course.createdAt}" pattern="hh:mm a (dd-MMM-yyyy)"
                                        var="formattedDate"/>
                        <td> ${formattedDate}</td>
                        <td class="${statusTextColor} fs-6 fw-bold">${statusText}</td>
                        <td>
                            <button type="button" class="col-2 btn btn-primary btn-sm me-2"
                                    onclick="location.href='courses/update?courseId=${course.id}'">
                                Edit
                            </button>
